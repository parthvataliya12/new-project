...
var App = function() {
    ...
    var uiYearCopy = function() {
        ...
        if ($date.getFullYear() === 2015) {
            ...
            $yearCopy.html('2014-' + $date.getFullYear().toString().substr(2,2));
        }
    };
...
$('#sidebar .sidebar-content ul.nav-main li span#inbox').prepend('<span id="change_unread_count" class="badge" style="text-align: left; position: absolute; padding-bottom: 5px; margin: -9px 0px 0px -18px;" id="inbox_unread"></span>');
chk_unread_msg();

$('#main-container #home_unread_msg').fadeOut().fadeIn();
	
});
setInterval(chk_unread_msg,10000);
...
function chk_unread_msg()
{
	...
	method: "POST",
	data:{flag:"chk_unread"},
	success:function(data){
			$('#sidebar .sidebar-content ul.nav-main li span#change_unread_count, ul.dropdown-menu li span#menu_unread_count').html(data);
			if($('#main-container #home_unread_msg_count').length)
			{
				$('#main-container #home_unread_msg_count,span#menu_unread_count').html(data);
					if($('#main-container #home_unread_msg_count').html()>0)
					{
					setInterval(flash_lable,1000);
					function flash_lable()
					{
						if($('#main-container #home_unread_msg').length)
						{
							$('#main-container #home_unread_msg').fadeOut().fadeIn();
						}
						if($('#main-container #home_unread_msg_count').length)
						{
							$('#main-container #home_unread_msg_count').fadeOut().fadeIn();;
						}
					}
				}
			}
